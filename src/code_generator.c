...
static int codegen_expression(CodeGenerator *gen, ASTNode *node)
{
    ...
    switch (node->type) {
        ...
        case AST_STRUCT_ACCESS: {
            /* struct.field - read a field from struct */
            ...
            for (size_t i = 0; i < struct_def->fields.count; ++i) {
                ...
                /* Each field is 8 bytes on the stack */
                field_offset -= 8;
            }
...
static int codegen_statement(CodeGenerator *gen, ASTNode *node)
{
    ...
    switch (node->type) {
        ...
        case AST_STRUCT_ASSIGN: {
            ...
            /* Store at struct_offset + field_offset */
                            ...
                            /* Check if it's an array access: name[index] */
                                ...
                                if (struct_var) {
                                    ...
                                    if (struct_def) {
                                        ...
                                        if (field_type == TYPE_STRING) {
                                            /* rax has string address, now load length from field_offset - 8 into rdx */
                                            /* Save string address */
                                            emit_push_rax(gen);
                                            /* Load string length */
